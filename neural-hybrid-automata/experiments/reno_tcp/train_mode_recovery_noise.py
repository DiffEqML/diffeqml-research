...
import pathlib
import wandb
from script_config import train_flows_noise_config
...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_dict("config", config=train_flows_noise_config)
...
def main(argv):
    ...
    for k in range(FLAGS.n_val_folds):
        ...
        # ground-truth modes
        gt_modes = []
        for traj_seg in data_sol_segments[k]:
            most_common_mode, _ = torch.mode(traj_seg[..., 0, 2:3], dim=0)
            gt_modes.append(most_common_mode)
        gt_modes = torch.cat(gt_modes).numpy()
